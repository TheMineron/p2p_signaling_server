...
from commands.settings.schemas import SetNicknameRequest, ParticipantRenamedResponse
from utils import (
    safe_send_json,
    broadcast_to_room,
    CommandContext,
...
@register_handler(
    "set_nickname",
    request_model=SetNicknameRequest,
    responses={
        "participant_renamed": (
                ParticipantRenamedResponse,
                "Уведомление всем (broadcast)"
        ),
        "error": (
                ErrorResponse,
                "Ошибка: пустой никнейм или участник не в комнате"
        )
    },
    description="Изменить отображаемый никнейм. Рассылается уведомление всем.",
    group="settings"
)
