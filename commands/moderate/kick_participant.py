from commands.moderate.schemas import KickParticipantRequest, KickedResponse, \
...
@register_handler(
    "kick_participant",
    request_model=KickParticipantRequest,
    responses={
        "kicked": (
                KickedResponse,
                "Отправляется удаляемому участнику перед закрытием соединения"
        ),
        "participant_left": (
                ParticipantUpdatedResponse,
                "Уведомление остальным участникам (broadcast)"
        ),
        "error": (
                ErrorResponse,
    ...
    description="Принудительное удаление участника (только для модератора).",
    group="moderate"
)
