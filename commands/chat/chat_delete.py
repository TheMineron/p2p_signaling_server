from commands.chat.schemas import ChatDeleteRequest, ChatDeletedResponse
...
@register_handler(
    "chat_delete",
    request_model=ChatDeleteRequest,
    responses={
        "chat_deleted": (
                ChatDeletedResponse,
                "Сообщение удалено (рассылка получателям)"
        ),
        "error": (
                ErrorResponse,
                "Ошибка: недостаточно прав, сообщение не найдено или не указан msg_id"
        )
    },
    description="Удаление сообщения (своего или любого, если модератор).",
    group="chat"
)
