...
from commands.chat.schemas import ChatPinRequest, ChatPinnedResponse
...
@register_handler(
    "chat_pin",
    request_model=ChatPinRequest,
    responses={
        "chat_pinned": (
                ChatPinnedResponse,
                "Сообщение закреплено (broadcast)"
        ),
        "error": (
                ErrorResponse,
    ...
    description="Закрепление сообщения (только для модератора).",
    group="chat"
)
