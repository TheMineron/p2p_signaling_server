...
from commands.chat.schemas import ChatSendRequest, ChatMessageResponse
...
@register_handler(
    ...
    responses={
        "chat_sent": (
                ChatMessageResponse,
                "Успешная отправка сообщения, рассылка получателям"
        ),
        "error": (
                ErrorResponse,
                "Ошибка: участник не в комнате или отсутствует текст"
        )
    },
    description="Отправка текстового сообщения (публичного или личного).",
    group="chat"
)
