...
from commands.chat.schemas import ChatMessageResponse, ChatReplyRequest
...
@register_handler(
    "chat_reply",
    request_model=ChatReplyRequest,
    responses={
        "chat": (
                ChatMessageResponse,
                "Ответ на сообщение (рассылка получателям)"
        ),
        "error": (
                ErrorResponse,
                "Ошибка: исходное сообщение не найдено или удалено"
        )
    },
    description="Ответ на существующее сообщение.",
    group="chat"
)
