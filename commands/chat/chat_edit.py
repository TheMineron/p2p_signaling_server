from commands.chat.schemas import ChatEditRequest, ChatEditedResponse
...
@register_handler(
    "chat_edit",
    request_model=ChatEditRequest,
    responses={
        "chat_edited": (
                ChatEditedResponse,
                "Сообщение отредактировано (рассылка получателям)"
        ),
        "error": (
                ErrorResponse,
                "Ошибка: не своё сообщение, отсутствуют поля или сообщение не найдено"
        )
    },
    description="Редактирование своего сообщения.",
    group="chat"
)
